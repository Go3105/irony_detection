...
for tokens in tokens_list:
    ...
    for index, token in enumerate(tokens):
        if token in sentiment_words: #もしもトークンが感情辞書に含まれている単語だったら(漢字)
            sentiment_word = token
            sentiment_index = sentiment_words.index(token)
            sentiment_score = sentiment_scores[sentiment_index]
            if sentiment_score > 0 and i > paradox_words_position and paradox_words_position != -1:
                pattern_matching = 1 
        elif token in sentiment_words_hiragana: #もしもトークンが感情辞書に含まれている単語だったら(ひらがな)
            sentiment_word = token
            sentiment_index = sentiment_words_hiragana.index(token)
            sentiment_score = sentiment_scores[sentiment_index]
            if sentiment_score > 0 and i > paradox_words_position and paradox_words_position != -1:
                pattern_matching = 1 
        else: #トークンが感情辞書に含まれていない単語だったら
            sentiment_word = token
            sentiment_score = 0 #感情スコアは0に
...
shift_columns = 1
with open("文章中に含まれる各単語の感情スコア.csv", "w", encoding="utf-8", newline="") as f: #csvファイルに書く
    writer = csv.writer(f)
    header = []
    for i in range(1 + max_sentiment_words_number + max_paradox_words_number + 3 + 2 + 2): #必要な数だけ列を確保
    ...
    # 各行のデータをずらして書き込む
    for c, scores_list, paradox_score_list, pattern_matching, final_words in zip(review_texts, sentiment_scores_list_list, paradox_scores_list_list, pattern_matching_list, final_words_score_list):
        shifted_row = [c] + [None] * shift_columns + scores_list + [None] *(max_sentiment_words_number - len(scores_list) + 1)  + paradox_score_list + [None] *(max_paradox_words_number - len(paradox_score_list) + 1) + [pattern_matching] + [None] * shift_columns + [final_words]
