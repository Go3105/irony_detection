...
for line in df.values:
    ...
    #encoding['labels'] = int(line[11]) # ラベルを追加(int()をつけないとエラーになる)
    #encoding['labels'] = line[11]-1 #「-1」が無いとエラーになる
    encoding['labels'] = 3 - line[13] # 2値バージョン「-2」が無いとエラーになる
...
accuracy_record = BertForSequenceClassification_pl.get_accuracy_value()
...
model = BertForSequenceClassification_pl(
    MODEL_NAME, num_labels=2, lr=0.8e-6
)
...
if result > 0.8:
    model.bert_sc.save_pretrained('./model_transformers_stance_manual/' +  str(result))
